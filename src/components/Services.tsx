...
export function Services() {
  ...
  const services = servicePosts.map((post) => ({
    ...
    color: post.metadata?.color || 'accent-emerald',
    rotation: post.metadata?.rotation || 'rotate-1',
  ...
  if (isLoading) {
    return (
      <section className="relative py-20 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #2d1810 0%, #1a0f08 30%, #0f0704 60%, #1a0f08 100%)' }}>
        <div className="w-8 h-8 border-4 border-amber-200/20 border-t-amber-200 rounded-full animate-spin" />
      </section>
    )
  }
    ...
    >
      ...
      <div className="relative bg-white p-4 pb-8 shadow-2xl cursor-pointer w-[260px] sm:w-[280px] max-w-[90vw]"
        ...
        <div className="relative">
          <h3 className="font-black text-lg text-gray-800 mb-3 leading-tight">{service.title}</h3>
  ...
  return (
    <section id="services" className="relative py-20" style={{
      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className={`w-full transform transition-all duration-1000 delay-600 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`} style={{ overflow: 'visible' }}>

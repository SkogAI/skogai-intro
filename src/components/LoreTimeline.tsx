          ...
          return (
            <div
              ...
              className={`border gentle-animation cursor-pointer group ${
                isExpanded
                  ? `${theme.entryExpandedBg}`
                  : 'border-foreground/5 hover:border-foreground/15'
              }`}
              style={isExpanded ? { borderColor: `color-mix(in srgb, ${theme.accentColor} 20%, transparent)` } : undefined}
                ...
                <div className="flex items-start gap-4">
                  {Icon && (
                    <div className={`mt-0.5 flex-shrink-0 gentle-animation ${
                      isExpanded ? theme.entryIconExpandedColor : 'text-foreground/20 group-hover:text-foreground/40'
                    }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-xs text-foreground/40 mt-1.5 leading-relaxed ${theme.fontFamily}`}>
                      {post.excerpt}
                    </p>

                    {isExpanded && (
                      <div className="mt-4 pt-4 border-t border-foreground/5">
                        <p className={`text-sm text-foreground/60 leading-[1.8] tracking-wide ${theme.fontFamily}`}>
                          {post.content}
                        </p>
                        {(post as any).slug && (
                          <Link
                            to={`/post/${(post as any).slug}`}
                            className={`inline-block mt-4 text-[10px] tracking-[0.2em] uppercase border-b pb-0.5 gentle-animation ${theme.readLinkColor} ${theme.fontFamily}`}
    ...
    <section className={`relative py-24 sm:py-32 ${theme.timelineBg}`}>
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="text-center mb-16 sm:mb-20">
          <span className={`text-[10px] tracking-[0.5em] uppercase block mb-4 ${theme.timelineHeaderColor} ${theme.fontFamily}`}>
            {theme.timelineHeaderText}
          </span>
          <h2 className={`text-3xl sm:text-4xl font-bold tracking-[0.06em] mb-4 ${theme.fontFamily}`}>
            {theme.timelineTitle}
          </h2>
          <p className={`text-xs text-foreground/40 max-w-lg mx-auto leading-relaxed ${theme.fontFamily}`}>
              ...
              {CATEGORIES.map((cat, index) => {
                ...
                return (
                  <button
                    ...
                    style={isActive ? { borderColor: `color-mix(in srgb, ${theme.accentColor} 20%, transparent)` } : undefined}
                  >
                    <span className={`text-[10px] font-bold tracking-wider tabular-nums gentle-animation ${
                      isActive ? theme.categoryActiveNumColor : 'text-foreground/20 group-hover:text-foreground/40'
                    } ${theme.fontFamily}`}>
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] text-foreground/25 hidden lg:block mt-0.5 ${theme.fontFamily}`}>
                        {cat.description}
                      </span>
                    </div>
                  </button>
            ...
            {CATEGORIES.map(cat => (
              <div key={cat.id} className={activeCategory === cat.id ? 'block' : 'hidden'}>
                <div className="mb-8 pb-6 border-b border-foreground/5">
                  <h3 className={`text-xl font-bold tracking-[0.08em] uppercase mb-2 ${theme.fontFamily}`}>
                    {cat.label}
                  </h3>
                  <p className={`text-xs text-foreground/40 tracking-wide ${theme.fontFamily}`}>
                    {cat.description}
                  </p>
                </div>
